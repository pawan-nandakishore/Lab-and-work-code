...
fileLoc2 = '/media/pawan/0B6F079E0B6F079E/Postdoc stuff/Metronome experiment/Videos/Temp rajeev data folder /123two45degree.MOV'
...
boxLower = np.array([100 ,25,  55],dtype=np.uint8)
boxUpper = np.array([150, 100 , 100],dtype=np.uint8)



box2Lower = np.array([95 ,35,  10],dtype=np.uint8)
box2Upper = np.array([150, 65 , 35],dtype=np.uint8)
...
with open('position_data_multiple_mets.csv', 'w') as csvfile:
        ...
        while(camera.isOpened()and writer.isOpened() ):
          frame= 0 
          mask2= 0 
          mask2 = 0
          ...
          mask3 = cv2.inRange(colour_frame,box2Lower,box2Upper)
...
#          plt.imshow(colour_frame)
         ## FIND CONTOURS- this part of the code gives us rect_contours and circle_contours 
          ...
          (rect_contours,hierarchy) = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL,
        		cv2.CHAIN_APPROX_SIMPLE)
          (circle_contours,hierarchy) = cv2.findContours(mask2.copy(), cv2.RETR_EXTERNAL,
        		cv2.CHAIN_APPROX_SIMPLE)
          
          (rect2_contours,hierarchy) = cv2.findContours(mask3.copy(), cv2.RETR_EXTERNAL,
        		cv2.CHAIN_APPROX_SIMPLE)
          ...
          for x in rectangle_indices[0]: 
              temp_rect_contours.append(rect_contours[x])
          ...
          rect_contours = [] 
          for x in final_rectangle_indices[0]: 
             rect_contours.append(temp_rect_contours[x])
#          temp_rect_contours = [] 
...
#          rect_contours = temp_rect_contours
          ...
          for x in rect2_contours: 
              rect2_areas.append(cv2.contourArea(x))
              
              
          maxvals = max(rect2_areas)    
          max_list = [i for i, j in enumerate(rect2_areas) if j == maxvals]
          rect2_contours = rect2_contours[max_list[0]]
         ...
         # If nothing is detected then crash the program 
          ...
          ## Moments of both the contours using which you can calculate the centroids ##
                  ...
                  #circle orientation  
                  (x,y),(MA,ma),angle = cv2.fitEllipse(rect_contours[single_met])
                  rect_orientation.append(angle)
          ...
          #a[a[:,1].argsort()]
#          rect_position[rect_position[:,1].argsort()]
          rect_2 = rect_position[:]
          rect_2 =[list(elem) for elem in rect_2]
          rect_2 = sorted(rect_2,key=lambda x: x[0])
          rect_position = [tuple(elem) for elem in rect_2]
          ...
          #centroid and orientation calculation for the director piece 
          
          rect2_moments = cv2.moments(rect2_contours)   
                  
          rect2_centroid_x = int(rect2_moments['m10']/rect2_moments['m00'])
          rect2_centroid_y = int(rect2_moments['m01']/rect2_moments['m00'])
          (x,y),(MA,ma),rect2_orientation = cv2.fitEllipse(rect2_contours)
          
          
          
          
          
         # this is drawing rectangles for all the metronomes and the bobs          
          for single_met in range(0,len(rect_contours)) :
                  ...
                  csvwrite.writerow((framenum,single_met,c_centroid_x,c_centroid_y, rect_centroid_x,rect_centroid_y,rect_orientation, x,y,w,h,xc,yc,wc,hc))
                 
          
          text4 = 'x ='+ str(rect2_centroid_x) +' '+'y ='+ str(rect2_centroid_y)  
          (rect2_x,rect2_y,rect2_w,rect2_h) = cv2.boundingRect(rect2_contours)
          cv2.rectangle(colour_frame, (rect2_x , rect2_y), (rect2_x + rect2_w, rect2_y + rect2_h),(0,0,255),2 )
          cv2.rectangle(colour_frame, (rect2_x , rect2_y), (rect2_x+sw, rect2_y-sh),(0,0,255),-2 ,)       
          cv2.putText(colour_frame, text4, (rect2_x,rect2_y-5), cv2.FONT_HERSHEY_TRIPLEX, 0.4,(0,255,0))        
